...
def get_columns_for_days(filters: Filters) -> list[dict]:
	...
	for d in dates_in_period:
		d = getdate(d)
		# sets days as numeric day value only, as per image
		label = cstr(d.day)
...
def get_rows(
	...
	if filters.filter_based_on == "Month":
		month_year = f"{cint(filters.month):02d}/{cstr(filters.year)[2:]}"
	else:
	...
	for shift, status_dict in employee_attendance.items():
		...
		for d in total_days:
			...
			if status in ["Present", "Work From Home"]:
				total_present += 1.0
			elif status in ["Half Day/Other Half Present", "Half Day"]:
				total_present += 0.5
				total_leaves += 0.5
